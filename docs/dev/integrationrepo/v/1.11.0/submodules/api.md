...
> **Integration tag:** `1.11.0` · **Submodule commit:** `f8647072b318746aadcd6d3a53ac5bf72ce2cac8`  
> **Repo:** git@github.com:pvarki/python-rasenmaeher-api.git  
> **Browse at this commit:** https://github.com/pvarki/python-rasenmaeher-api/tree/f8647072b318746aadcd6d3a53ac5bf72ce2cac8
...
This application can be configured with environment variables. Below is
a example list of available variables. You can find all available
variables here
[https://github.com/pvarki/python-rasenmaeher-api/blob/main/src/rasenmaeher_api/settings.py](https://github.com/pvarki/python-rasenmaeher-api/blob/main/src/rasenmaeher_api/settings.py).
...
| RM_KEYCLOAK_SERVER_URL    | None                                        | Keycloak server url ([http://1234:8080/auth](http://1234:8080/auth))                 |
...
You can create <span class="title-ref">.env</span> file in the root
directory and place all environment variables here.

All environment variables should start with
<span class="title-ref">RM\_</span> prefix.

For example if you see in your "rasenmaeher_api/settings.py" a variable
named like <span class="title-ref">random_parameter</span>, you should
provide the "RM_RANDOM_PARAMETER" variable to configure the value. This
behaviour can be changed by overriding
<span class="title-ref">env_prefix</span> property in
<span class="title-ref">rasenmaeher_api.settings.Settings.Config</span>.

An example of .env file:
`` `bash RM_RELOAD="True" RM_PORT="8000" RM_ENVIRONMENT="dev" ``\`
...
Integration APIs are always https in port 4625. Default location for the
manifest is /pvarki/kraftwerk-rasenmaeher-init.json
...
| Dummy   | GET    | /api/v1/enroll/deliver/&#123;id_string&#125; | NA                                                           | &#123;'dl_link':"&#123;[http://here.be/zip](http://here.be/zip)&#125;"&#125;                                                | Deliver download link for enrollment zip                                                              |
...
\# REQUEST A NEW CERTIFICATE USING CSR (requires cfssl backend for the
api container)
...
\# REQUEST A NEW CERTIFICATE WITHOUT CSR (requires cfssl backend for the
api container)
...
The cfssl used behind API listens this kind of stuff
[https://github.com/cloudflare/cfssl/blob/master/doc/api/endpoint_newcert.txt](https://github.com/cloudflare/cfssl/blob/master/doc/api/endpoint_newcert.txt)
...
For more controlled deployments and to get rid of "works on my computer"
-syndrome, we always make sure our software works under docker.

It's also a quick way to get started with a standard development
environment.
...
And also the exact way for forwarding agent to running instance is
different on OSX:
...
CI use the "tox" target in the Dockerfile:
...
NOTE: This will not work from the git submodule directory in the
integration repo, docker tests must be run from the original repo.

### Production docker

There's a "production" target as well for running the application,
...
Remember to activate your virtualenv whenever working on the repo, this
